...
int JohorLinearSearchTutor(Tutor* tutorArray, int tutorID) {
	...
	//check if there is any tutor data in the array matches with the user input and the tuition name is JOHOR
...
void JohorSearchTutorByRating(Tutor tutorArray[])
{
	...
	//check if any tutor rating is equal to user input and and tuition name is JOHOR
	for (int i = 0; i < size; i++)
	{
	...
	else
	{
		...
		//print all tutor with user input rating and tuition name is JOHOR
		for (int i = 0; i < size; i++)
		{
